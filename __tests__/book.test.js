...
describe("PUT /books/:id", function () {
  test("Updates a single book", async function () {
    const response = await request(app)
        .put(`/books/${book_isbn}`)
        .send({
          amazon_url: "https://amazon/drseuss.com",
          author: "Seuss",
          language: "english",
          pages: 20,
          publisher: "ABC Books",
          title: "Updated red fish book",
          year: 1962
        });
    expect(response.body.book).toHaveProperty("isbn");
    expect(response.body.book.title).toBe("Updated red fish book");
  });
